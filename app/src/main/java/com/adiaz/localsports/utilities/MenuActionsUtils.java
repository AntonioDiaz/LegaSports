...
public class MenuActionsUtils {
	...
	public static void showMatchLocation(Context context, Match match) {
		Court court = LocalSportsCourts.obteinTownCourt(context, match.idSportCenter());
		if (court==null) {
	...
	public static void shareMatchDetails(Activity activity, Match match, Competition competition) {
		...
		String sport = LocalSportsUtils.getStringResourceByName(activity, competition.sportName());
		String category = LocalSportsUtils.getStringResourceByName(activity, competition.categoryName());
		String dateStr = match.obtainDateStr(activity);
		SharedPreferences preferences = getDefaultSharedPreferences(activity);
		String town = preferences.getString(LocalSportsConstants.KEY_TOWN_NAME, null);
	...
	public static void addMatchEvent(Context context, Match match, Competition competition) {
		if (match.date()==null || match.date().getTime()==0) {
			...
			String sport = LocalSportsUtils.getStringResourceByName(context, competition.sportName());
			String category = LocalSportsUtils.getStringResourceByName(context, competition.categoryName());
