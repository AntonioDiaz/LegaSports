...
public class FavoriteTeamAdapter extends RecyclerView.Adapter<FavoriteTeamAdapter.ViewHolder> {
	...
	public void onBindViewHolder(ViewHolder holder, int position) {
		...
		if (match == null
				|| match.teamLocal().equals(LocalSportsConstants.UNDEFINDED_FIELD)
				|| match.teamVisitor().equals(LocalSportsConstants.UNDEFINDED_FIELD)) {
			...
			if (!match.isDateDefined()) {
				LocalSportsUtils.disableImageButton(holder.ibCalendar);
			}
			if (!match.isCourtDefinded(context)) {
				LocalSportsUtils.disableImageButton(holder.ibLocation);
			}
		}
